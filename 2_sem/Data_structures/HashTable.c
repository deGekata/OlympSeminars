...
int main() {
    ...
    Hashmap** maps = (Hashmap**) calloc(10, sizeof(Hashmap*));
    int* map_ex = (int*) calloc(10, sizeof(int*));

    char buff[30];
    char buff2[30];
    size_t table_num = 0;
    for (size_t it = 0; it < 10; ++it) {
        maps[it] = HashmapCreate(strHashCode);
        map_ex[it] = 2;
    }
    
    while ((scanf("%25s %lu", buff, &table_num)) != EOF
        // && cnt != 0
    ) {
        // cnt--;
         if (strcmp(buff, "destroy") == 0) {
            ...
            if (map_ex[table_num] == 1) assert(0 && "wtf");
            // root = AVLTreePush(root, buff, buff2);
            map_ex[table_num] = 1;
        } else if (strcmp(buff, "insert") == 0) {
            scanf("%25s", buff);
            scanf("%25s", buff2);
            if (map_ex[table_num] == 0) continue;
            Hashmap_insert(maps + table_num, buff, buff2);
            // root = AVLTreePush(root, buff, buff2);
        } else if (strcmp(buff, "find") == 0) {
            scanf("%25s", buff);
            HValue ret_val = get(maps[table_num], buff);
            if (ret_val == NULL) {
                printf("MISSING\n");
            } else {
                printf("%s\n", ret_val);
            ...
            scanf("%25s", buff);
            if (map_ex[table_num] == 0) continue;
            Entry* ret_val = Hashmap_remove(maps[table_num], buff);
            if (ret_val != NULL) {
                EntryDtor(ret_val);
                free(ret_val);
            }
