...
int compare_dot(const void* x1_, const void* x2_) {
    Dot* x1 = (Dot*) x1_;
    Dot* x2 = (Dot*) x2_;

    if (x1->x != x2->x) return (x1->x - x2->x) / abs(x1->x - x2->x);

    if (x1->y != x2->y) return (x1->y - x2->y) / abs(x1->y - x2->y);

    if (x1->z != x2->z) return (x1->z - x2->z) / abs(x1->z - x2->z);
...
int main() {
    ...
    Dot* arr = (Dot*) calloc(cnt, sizeof(Dot));

    for (int it = 0; it < cnt; ++it) {
        scanf("%ld %ld %ld", &(arr[it].x), &(arr[it].y), &(arr[it].z));
    }

    qsort(arr, cnt, sizeof(Dot), compare_dot);
    ...
    for (int it = 0; it <= cnt / 2; ++it) {
        if (
            (arr[it].x + arr[cnt - it - 1].x != f_sum.x) ||
            (arr[it].y + arr[cnt - it - 1].y != f_sum.y) ||
            (arr[it].z + arr[cnt - it - 1].z != f_sum.z)
            )
        {
            printf("No");
            return 0;
